#!/usr/bin/env python
"""train_bertimbau_v4.py -- treino/avaliacao/calibracao/retomada do BERTimbau v4.
...
Contrato e decisoes: `models/v4/spec_treino_v4.md` + `models/v4/PLANO_FT_V4_T4.md`.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--prepare", action="store_true",
                    help="delega para models.v4.prepare_v4 e sai (local)")
    ap.add_argument("--sanitized", default="FakenewsBR_sanitized_v4.csv")
    ap.add_argument("--labels", default="FakenewsBR_v4_labels.csv")
    ap.add_argument("--provenance", default="FakenewsBR_v4_provenance.csv")
    ap.add_argument("--out-dir", default="models/v4/processed",
                    help="out-dir do --prepare (nao confundir com --out)")
    ap.add_argument("--data", default=None)
    ap.add_argument("--splits", default=None)
    ap.add_argument("--split-col", default="bal_iid")
    ap.add_argument("--eval-col", default="bal_iid")
    ...
    ap.add_argument("--dfr-weights", choices=["off", "cell"], default="off")
    ...
    ap.add_argument("--ablate", choices=["none", "fullpool", "dfr", "maxlen256",
...
ABLATE_PRESETS = {
    "none": {},
    "fullpool": {"split_col": "full_iid", "dfr_weights": "cell", "epochs": 2,
                 "eval_col": "bal_iid"},
    ...
    "classweights": {"class_weights": True},
    "llmoff": {"split_col": "full_iid", "dfr_weights": "cell",
               "drop_tiers": "llm_local,corroborated"},
}
...
def run_prepare(args) -> int:
    ...
    sys.path.insert(0, str(root))
    try:
        from models.v4 import prepare_v4
    except Exception as e:
        print("[prepare] nao consegui importar models.v4.prepare_v4. No Colab, "
              "gere o parquet localmente e envie v4_pool.parquet/v4_splits.parquet. "
              f"({e})")
        return 2
    ...
    return int(prepare_v4.main(argv))
...
def main(argv=None) -> int:
    ...
    if args.data is None or args.splits is None:
        ...
        return 2
    if args.split_col == "full_iid" and args.eval_col == "full_iid":
        print("[ERRO] split full_iid nao tem teste; use --eval-col bal_iid "
              "--eval-value test (teste canonico, holdout do full_iid)")
        return 2
    ...
    if args.run_id is None:
        ...
        if args.smoke:
            args.run_id += "_smoke"
    if args.out is None:
        args.out = str(Path("models/v4/artifacts") / args.run_id)
    ...
    if missing:
        ...
        return 2
    canon_test_rids = set(splits.loc[splits["split_bal_iid"] == "test", "rid"]) \
        if "split_bal_iid" in splits.columns else set()
    ...
    if "has_ufffd" in pool.columns:
        n_uff = int(pool["has_ufffd"].sum())
        if n_uff:
            print(f"[E2] removendo {n_uff} linhas has_ufffd (defensivo)")
            pool = pool[~pool["has_ufffd"]].reset_index(drop=True)
    ...
    canon_test = pd.DataFrame({"rid": sorted(canon_test_rids)})
    inter = set(train_df["rid"]) & set(canon_test["rid"])
    print(f"[E2] interseccao treino x teste canonico (bal_iid.test): {len(inter)}")
    if args.split_col == "full_iid" and inter:
        print("[ERRO] full_iid deve respeitar o holdout do teste canonico")
        return 2
    ...
    # probe de VRAM (E9) -- opt-in, antes de treinar
    if args.probe_max_length is not None:
        if device.type != "cuda":
            print("[E9] --probe-max-length ignorado sem CUDA (VRAM de pico = 0)")
        else:
    ...
    if args.resume:
        ...
        if not last_dir.exists():
            print("[E7] aviso: last/ nao existe; comecando do zero")
        else:
            cfg_path = out_dir / "run_config.json"
            compat = ["data_sha256", "splits_sha256", "split_col", "max_length",
                      "freeze_layers", "model", "seed", "dfr_weights", "class_weights"]
            diffs = []
            if cfg_path.exists():
    ...
    eval_kind = "ood" if args.eval_col == "bal_ood_wa" else "test"
    ...
    for cut in ("channel", "publisher", "era", "label_tier"):
        tab = per_group(eval_df, y_ev, p_cal_ev, col=cut)
        cuts[cut] = jsonable(tab.to_dict(orient="records")) if not tab.empty else []
        ...
        if tok_s > 0 and default_tokens:
            for sp in (10, 30):
                ...
                print(f"    speedup {sp:>2}x -> ~{eta/60:.1f} min/epoca "
                      f"(R0 192: {default_tokens/1e6:.2f}M tokens pagos/epoca; "
                      f"3 epocas ~{3*eta/60:.0f} min)")
        else:
