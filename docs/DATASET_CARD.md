...
- config_name: default
  data_files:
  - split: train
    path: data/FakenewsBR_sanitized_v6.csv
dataset_info:
  features:
  - name: rid
    dtype: int64
...
|---|---:|---|
| `FakenewsBR_sanitized_v6.csv` | 297.672 | dataset principal, 23 colunas |
| `FakenewsBR_v6_public.csv` | 297.672 | variante para publicação com PII mascarada (`scrub_pii.py`) |
| `FakenewsBR_v6_provenance.csv` | 258.206 | proveniência das linhas novas (`rid`, `label_source`, `text_role`, `publisher`, `lang_variant`, `collector`, `source_url`, `collected_at`, `rating_norm`, `mentions_ai`) |
| `FakenewsBR_v6_labels.csv` | 297.672 | camadas de rótulo (`label_tier`, `auto_label`, `verified_label`, `confidence`, `method`, `evidence`, `train_label`) |
...
d = data.load(csv="data/FakenewsBR_sanitized_v6.csv",
...
python -m investigation.expansion.merge_and_audit --news-max-ratio 6 \
  ...
  --out data/FakenewsBR_sanitized_v6.csv --provenance data/FakenewsBR_v6_provenance.csv

# 4) camadas de rotulo
python -m investigation.expansion.apply_verification \
--base data/FakenewsBR_v6_provenance.csv --v2 data/FakenewsBR_sanitized_v6.csv \
--out data/FakenewsBR_v6_labels.csv
```
